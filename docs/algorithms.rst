...
Consider now the :math:`r`-partitions of the integer :math:`n`, there are :math:`{n+r-1 \choose r-1}` of such partitions. Let :math:`e=(e_0,\ldots,e_{r-1})` be an even :math:`r`-partition (i.e. an :math:`r`-partition where each element is even), and call :math:`\mathcal{E}_{n,r}` the set of all even :math:`r`-partitions, and define :math:`\lambda_e` to be the coefficient of :math:`x_0^{e_0}\ldots x_{r-1}^{p_{e-1}}` in the polynomial :math:`q(x_0,\ldots,x_{r-1})`. Then one can write the hafnian :math:`\bm{A} = \bm{G} \bm{G}^T` as
