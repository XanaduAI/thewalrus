...
void
powtrace (double z[], int n, double traces[], int l)
{
    /*
...
double
do_chunk (double mat[], int n, unsigned long long int X,
    ...
    for (unsigned long long int x = X; x < X + chunksize; x++)
    {
        ...
        for (i = 1; i <= n / 2; i++)
        {
            ...
            for (j = 1; j <= (n / (2 * i)); j++)
            {
                ...
                {
                    comb[1 - cntindex][k - 1] =
                        comb[1 - cntindex][k - 1] + comb[cntindex][k - i * j -
                                1] * powfactor;
                }
            }
        }
...
void
haf (double mat[], int n, double res[])
{
    /*
...
double do_chunk_loops(double mat[], double C[], double D[], int n, unsigned long long int X, unsigned long long int chunksize)
{
    /*
      This function calculates adds parts X to X+chunksize of Cygan and Pilipczuk formula for the
      Hafnian of matrix mat
    ...
    #pragma omp parallel
    {
        ...
        for(unsigned long long int x = X * chunksize; x < (X + 1)*chunksize; x++)
        {
            ...
            double B[sum * sum], B_powtrace[sum * sum];
            double C1[sum], D1[sum];
            ...
            for(i = 0; i < sum; i++)
            {
                for(j = 0; j < sum; j++)
                {
                    B[i * sum + j] = (double) mat[pos[i] * n + ((pos[j]) ^ 1)];
                    B_powtrace[i * sum + j] = (double) mat[pos[i] * n + ((pos[j]) ^ 1)];
                }
                C1[i] = C[pos[i]];
                D1[i] = D[pos[i]];
            }
...
double dhafnian_loops(double *mat, int n)
{
    return hafnian_loops((double *)mat, n);
}

void haf_loops(double mat[], int n, double res[])
{
    /*
      This is a wrapper for the function hafnian. Instead of returning the Hafnian of the matrix
      mat by value it does by reference with two doubles for the real an imaginary respectively.
    */
    ...
    double result = hafnian_loops(mat, n);
    res[0] = creal(result);
    res[1] = cimag(result);
}
